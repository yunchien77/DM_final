...
FEATURE_VERSION = 15  # +35 standardized physical-signal features (shift-invariant)

# Expose the 7 standardized physical drought signals (proxy._proxy_signals_window:
# vpd-anomaly-z, precip-deficit-z, dry-day-frac, heat-dd-ratio, humidity-z, tmp-z,
# dtr-z) directly to LightGBM at all 5 WINDOW_SIZES (= 35 features). They are
# currently only consumed — compressed — by the proxy Ridge. Each is a z-score /
# ratio vs region climatology, so the +6C/-25%-precip test maps back into the
# train-observed range instead of saturating the tree on out-of-support raw values.
USE_PHYSICAL_SIGNAL_FEATURES = True
PHYS_SIGNAL_NAMES = ["vpd_z", "precdef_z", "dryfrac", "hdd_ratio", "hum_z", "tmp_z", "dtr_z"]
