...
from config import (
    ...
    SCORE_ACF_BASE,
    USE_PHYSICAL_SIGNAL_FEATURES, PHYS_SIGNAL_NAMES,
)
from proxy import get_proxy_scores, _proxy_signals_window
...
def build_feature_names() -> list[str]:
    ...
    # ── 11. Standardized physical drought signals (shift-invariant), full resolution ──
    if USE_PHYSICAL_SIGNAL_FEATURES:
        for w in WINDOW_SIZES:
            for nm in PHYS_SIGNAL_NAMES:
                names.append(f"psig_{nm}_{w}d")
    return names
...
def make_features(
    ...
    # ── 11. Standardized physical drought signals (shift-invariant), full resolution ──
    # The 7 proxy signals (vpd_z, precdef_z, dryfrac, hdd_ratio, hum_z, tmp_z, dtr_z)
    # at each of the 5 WINDOW_SIZES. Exposed directly to LightGBM (not just compressed
    # into the proxy Ridge) so the tree can use them nonlinearly. z-scores/ratios vs
    # region climatology → in-support under the +6C/-25%-precip test shift.
    if USE_PHYSICAL_SIGNAL_FEATURES:
        for w in WINDOW_SIZES:
            tail = window_arr[-w:] if n >= w else window_arr
            feats.extend(_proxy_signals_window(tail, rb).tolist())
