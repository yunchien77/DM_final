...
weights) with the linear SSM model. Artifacts isolated (patchtst_ssm*.pt / *_ssm* norm+regmap).
...
from patchtst.ssm import build_ssm_model_from_config

SEED = int(os.environ.get("SSM_SEED", SEED))
_SUF = os.environ.get("SSM_SUF") or (f"_s{SEED}" if os.environ.get("SSM_SEED") else "")
PT_SSM_MODEL_PATH = PT_MODELS_DIR / f"patchtst_ssm{_SUF}.pt"
PT_SSM_METRICS_PATH = PT_MODELS_DIR / f"metrics_ssm{_SUF}.json"
SSM_NORM_PATH = PT_NORM_PATH.with_name(PT_NORM_PATH.stem + "_ssm" + _SUF + PT_NORM_PATH.suffix)
SSM_REGMAP_PATH = PT_REGION_MAP_PATH.with_name(PT_REGION_MAP_PATH.stem + "_ssm" + _SUF + PT_REGION_MAP_PATH.suffix)
VAL_FRAC = 0.10
...
def main():
    ...
    print(f"Device: {device}  [Linear-SSM]  model={PT_SSM_MODEL_PATH.name}", flush=True)
    ...
    del train_daily
    save_region_map(region_ids, path=SSM_REGMAP_PATH)
    ...
    save_channel_norm(mean, std, path=SSM_NORM_PATH)
    ...
    model = build_ssm_model_from_config(n_regions=len(region_ids)).to(device)
    print(f"[3] Linear-SSM params={sum(p.numel() for p in model.parameters())/1e6:.3f}M", flush=True)
    ...
    for ep in range(EPOCHS):
        ...
        if macro < best - 1e-4:
            ...
            torch.save({"model_state": model.state_dict(), "epoch": ep + 1, "val_macro_mae": macro}, PT_SSM_MODEL_PATH)
            print(f"    new best -> {PT_SSM_MODEL_PATH}", flush=True)
        else:
            patience += 1
    ...
    json.dump({"best_val_macro_mae": best, "best_epoch": best_ep, "history": hist},
              open(PT_SSM_METRICS_PATH, "w"), indent=2)
