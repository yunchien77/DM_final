"""Train the linear SSM (reuses the DLinear/GRU pipeline + the val fix).

Same setup as train_pt_dlinear/gru (representative random 90/10 val, global-norm input, severity
weights) with the linear SSM model. Artifacts isolated (patchtst_auto*.pt / *_ssm* norm+regmap).

Usage: /mnt/1stHDD/juiyun/miniforge3/envs/DMFP/bin/python -u -m patchtst.train_pt_ssm
"""
...
from patchtst.autoformer import build_autoformer_model_from_config

SEED = int(os.environ.get("AUTO_SEED", SEED))
_SUF = os.environ.get("AUTO_SUF") or (f"_s{SEED}" if os.environ.get("AUTO_SEED") else "")
PT_AUTO_MODEL_PATH = PT_MODELS_DIR / f"patchtst_auto{_SUF}.pt"
PT_AUTO_METRICS_PATH = PT_MODELS_DIR / f"metrics_auto{_SUF}.json"
AUTO_NORM_PATH = PT_NORM_PATH.with_name(PT_NORM_PATH.stem + "_auto" + _SUF + PT_NORM_PATH.suffix)
AUTO_REGMAP_PATH = PT_REGION_MAP_PATH.with_name(PT_REGION_MAP_PATH.stem + "_auto" + _SUF + PT_REGION_MAP_PATH.suffix)
VAL_FRAC = 0.10
...
def main():
    ...
    pf = open(PT_LOGS_DIR / f"progress_ssm_{_ts}.log", "w")
    print(f"Device: {device}  [Autoformer]  model={PT_AUTO_MODEL_PATH.name}", flush=True)

    if not PT_TRAIN_PKL.is_file():
        raise FileNotFoundError(f"need cached {PT_TRAIN_PKL}")
    t0 = time.time(); train_daily = pd.read_pickle(PT_TRAIN_PKL)
    ...
    del train_daily
    save_region_map(region_ids, path=AUTO_REGMAP_PATH)
    ...
    print(f"[2] train={len(train_anchors):,} val={len(val_anchors):,}  train target-mean={tgt_flat.mean():.3f}", flush=True)
    ...
    save_channel_norm(mean, std, path=AUTO_NORM_PATH)
    ...
    eff_batch = BATCH_SIZE * max(1, n_gpus)        # DataParallel splits this across GPUs
    train_loader = DataLoader(WithIdx(train_ds), batch_size=eff_batch, shuffle=True,
                              num_workers=NUM_WORKERS_LOADER, pin_memory=True, drop_last=True,
                              persistent_workers=NUM_WORKERS_LOADER > 0)
    val_loader = DataLoader(val_ds, batch_size=eff_batch * 2, shuffle=False,
                            num_workers=NUM_WORKERS_LOADER, pin_memory=True,
                            persistent_workers=NUM_WORKERS_LOADER > 0)

    model = build_autoformer_model_from_config(n_regions=len(region_ids)).to(device)
    print(f"[3] Autoformer params={sum(p.numel() for p in model.parameters())/1e6:.3f}M", flush=True)
    if n_gpus > 1:
        model = torch.nn.DataParallel(model)
        print(f"[3] DataParallel across {n_gpus} GPUs; eff_batch={eff_batch}", flush=True)
    lr_eff = LR * (n_gpus ** 0.5) if n_gpus > 1 else LR     # sqrt batch-scaling for the larger batch
    optim = torch.optim.AdamW(model.parameters(), lr=lr_eff, weight_decay=WEIGHT_DECAY)
    ...
    for ep in range(EPOCHS):
        ...
        if macro < best - 1e-4:
            ...
            _sd = (model.module if isinstance(model, torch.nn.DataParallel) else model).state_dict()
            torch.save({"model_state": _sd, "epoch": ep + 1, "val_macro_mae": macro}, PT_AUTO_MODEL_PATH)
            print(f"    new best -> {PT_AUTO_MODEL_PATH}", flush=True)
        else:
            patience += 1
    ...
    json.dump({"best_val_macro_mae": best, "best_epoch": best_ep, "history": hist},
              open(PT_AUTO_METRICS_PATH, "w"), indent=2)
