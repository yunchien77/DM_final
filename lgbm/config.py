...
PRUNED_FEATURE_LIST_PATH = Path(__file__).parent / "feature_cols_pruned.json"
...
LGBM_PARAMS = dict(
    ...
    n_estimators=int(os.environ.get("LGBM_N_EST", "3000")),  # LGBM_N_EST=N for quick runs (full phase1b used 8000)
    learning_rate=0.02,
    num_leaves=63,
    ...
    reg_alpha=0.1,
    reg_lambda=0.1,
    n_jobs=int(os.environ.get("LGBM_THREADS", "16")),  # cores per LightGBM (horizons train sequentially)
    random_state=42,
    verbose=-1,
)
...
N_WORKERS = max(1, min(int(os.environ.get("N_WORKERS_CAP", "16")), os.cpu_count() - 1))  # feature-build region pool
