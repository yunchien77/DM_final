"""Train the GRU regressor (reuses the PatchTST/DLinear data pipeline + the val fix).

Same setup as train_pt_dlinear (representative random 90/10 val, global-norm input, severity
weights) but with the GRU model. GRU artifacts are isolated (patchtst_gru*.pt / *_gru* norm+regmap)
so they never collide with the DLinear files.

Usage: /mnt/1stHDD/juiyun/miniforge3/envs/DMFP/bin/python -u -m patchtst.train_pt_gru
"""
...
from patchtst.gru import build_gru_model_from_config

SEED = int(os.environ.get("GRU_SEED", SEED))
_SUF = os.environ.get("GRU_SUF") or (f"_s{SEED}" if os.environ.get("GRU_SEED") else "")
PT_GRU_MODEL_PATH = PT_MODELS_DIR / f"patchtst_gru{_SUF}.pt"
PT_GRU_METRICS_PATH = PT_MODELS_DIR / f"metrics_gru{_SUF}.json"
GRU_NORM_PATH = PT_NORM_PATH.with_name(PT_NORM_PATH.stem + "_gru" + _SUF + PT_NORM_PATH.suffix)
GRU_REGMAP_PATH = PT_REGION_MAP_PATH.with_name(PT_REGION_MAP_PATH.stem + "_gru" + _SUF + PT_REGION_MAP_PATH.suffix)
VAL_FRAC = 0.10
...
def main():
    ...
    pf = open(PT_LOGS_DIR / f"progress_gru_{_ts}.log", "w")
    print(f"Device: {device}  [GRU]  model={PT_GRU_MODEL_PATH.name}", flush=True)
    ...
    del train_daily
    save_region_map(region_ids, path=GRU_REGMAP_PATH)
    ...
    save_channel_norm(mean, std, path=GRU_NORM_PATH)
    ...
    model = build_gru_model_from_config(n_regions=len(region_ids)).to(device)
    print(f"[3] GRU params={sum(p.numel() for p in model.parameters())/1e6:.3f}M", flush=True)
    ...
    for ep in range(EPOCHS):
        ...
        if macro < best - 1e-4:
            ...
            torch.save({"model_state": model.state_dict(), "epoch": ep + 1, "val_macro_mae": macro}, PT_GRU_MODEL_PATH)
            print(f"    new best -> {PT_GRU_MODEL_PATH}", flush=True)
        else:
            patience += 1
    ...
    json.dump({"best_val_macro_mae": best, "best_epoch": best_ep, "history": hist},
              open(PT_GRU_METRICS_PATH, "w"), indent=2)
