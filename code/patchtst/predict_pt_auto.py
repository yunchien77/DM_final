"""Autoformer inference -> submission. Mirrors predict_pt_dlinear/gru (concat train+test history
-> last 260 weeks, global norm) but loads the linear SSM.

Usage: /mnt/1stHDD/juiyun/miniforge3/envs/DMFP/bin/python -u -m patchtst.predict_pt_ssm -o /abs/out.csv
"""
...
from patchtst.predict_pt import TestDailyDataset, _load_test_daily
from patchtst.autoformer import build_autoformer_model_from_config
from patchtst.train_pt_auto import PT_AUTO_MODEL_PATH, AUTO_NORM_PATH, AUTO_REGMAP_PATH

AUTO_SUBMISSION_PATH = Path(__file__).parent.parent.parent / "submission_auto.csv"


def main(output_path=None):
    output_path = Path(output_path or AUTO_SUBMISSION_PATH)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    _ts = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    pf = open(PT_LOGS_DIR / f"predict_ssm_{_ts}.log", "w")
    ...
    mean, std = load_channel_norm(path=AUTO_NORM_PATH)
    region_map = load_region_map(path=AUTO_REGMAP_PATH)
    ...
    model = build_autoformer_model_from_config(n_regions=len(region_map)).to(device)
    ckpt = torch.load(PT_AUTO_MODEL_PATH, map_location=device)
    model.load_state_dict(ckpt["model_state"]); model.eval()
    print(f"[model] loaded {PT_AUTO_MODEL_PATH} (epoch {ckpt['epoch']}, val {ckpt['val_macro_mae']:.4f})", flush=True)
