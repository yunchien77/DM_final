"""GRU inference -> submission. Mirrors predict_pt_dlinear (concat train+test history ->
last 260 weeks, global norm) but loads the GRU model.

Usage: /mnt/1stHDD/juiyun/miniforge3/envs/DMFP/bin/python -u -m patchtst.predict_pt_gru -o /abs/out.csv
"""
...
from patchtst.train_pt_gru import PT_GRU_MODEL_PATH, GRU_NORM_PATH, GRU_REGMAP_PATH

GRU_SUBMISSION_PATH = Path(__file__).parent.parent.parent / "submission_gru.csv"


def main(output_path=None):
    output_path = Path(output_path or GRU_SUBMISSION_PATH)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    _ts = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    pf = open(PT_LOGS_DIR / f"predict_gru_{_ts}.log", "w")
    ...
    mean, std = load_channel_norm(path=GRU_NORM_PATH)
    region_map = load_region_map(path=GRU_REGMAP_PATH)
    ...
    model = build_gru_model_from_config(n_regions=len(region_map)).to(device)
    ckpt = torch.load(PT_GRU_MODEL_PATH, map_location=device)
    model.load_state_dict(ckpt["model_state"]); model.eval()
    print(f"[model] loaded {PT_GRU_MODEL_PATH} (epoch {ckpt['epoch']}, val {ckpt['val_macro_mae']:.4f})", flush=True)
