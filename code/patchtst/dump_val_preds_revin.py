"""Dump per-anchor val predictions for the RevIN-equipped PatchTST.

Mirrors dump_val_preds.py but loads the RevIN model and runs the val loader
with apply_global_norm=False.
"""
...
OUT_PATH = PT_MODELS_DIR / "val_preds_patchtst_revin.npz"
...
def main():
    ...
    print("[Stage 4] loading channel norm (unused for RevIN) + RevIN model checkpoint")
    ...
    model = build_revin_model_from_config(n_regions=len(region_ids)).to(device)
    ckpt = torch.load(PT_REVIN_MODEL_PATH, map_location=device)
    model.load_state_dict(ckpt["model_state"])
    model.eval()
    print(f"  loaded {PT_REVIN_MODEL_PATH}  "
