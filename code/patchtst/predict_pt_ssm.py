"""Linear-SSM inference -> submission. Mirrors predict_pt_dlinear/gru (concat train+test history
...
from patchtst.train_pt_ssm import PT_SSM_MODEL_PATH, SSM_NORM_PATH, SSM_REGMAP_PATH

SSM_SUBMISSION_PATH = Path(__file__).parent.parent.parent / "submission_ssm.csv"


def main(output_path=None):
    output_path = Path(output_path or SSM_SUBMISSION_PATH)
    ...
    mean, std = load_channel_norm(path=SSM_NORM_PATH)
    region_map = load_region_map(path=SSM_REGMAP_PATH)
    ...
    model = build_ssm_model_from_config(n_regions=len(region_map)).to(device)
    ckpt = torch.load(PT_SSM_MODEL_PATH, map_location=device)
    model.load_state_dict(ckpt["model_state"]); model.eval()
    print(f"[model] loaded {PT_SSM_MODEL_PATH} (epoch {ckpt['epoch']}, val {ckpt['val_macro_mae']:.4f})", flush=True)
